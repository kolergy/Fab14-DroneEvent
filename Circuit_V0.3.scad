...
module corner(re,w,a) { 
    translate([0,w, 0]) rotate([0,0,-90]) {
            color("blue") rotate_extrude(angle=a, convexity = 10, $fn=10*q)
        translate([re, 0, 0]) {
            square([th,hi]);
...
module line(le, w, a){
    rotate([0,0,a]) {
        color("blue") translate([0, 0, 0]) cube([le,th,hi]);
        color("darkslategrey") translate([0, 0, 0]) cube([le,w,0.1]);
        color("blue") translate([0,w, 0]) cube([le,th,hi]);
...
color("red") translate([0,0,-2]) cube([12000,6000,1]);   // Race area
...
translate([ 3000,1500,0]) start(wm);
translate([ 4500,1000,0]) arrow(wm);

translate([ 3000,00,0]) mediatheque();
translate([ 3000,1500,1006]) start(wm);
translate([ 6800,2200,0]) rotate([0,0,-15])    marteuleur();
translate([ 8000,480,0]) rotate([0,0, 0])    gateA(wm+50,2000);
translate([ 11500,3700,0]) rotate([0,0,85])    gateB(wm+50,2000,1000);
...
translate([5500,3800,0]) rotate([0,0,-25]) building(430);
